...
const ArticleDetail = () => {
  ...
  const articleQuery = useQuery<BlogArticulo>({
    queryKey: ['articles', id],
    queryFn: () => getArticleById(id ?? ''),
    enabled: Boolean(id),
  });

  if (articleQuery.isLoading) return <div className="text-center mt-10">Cargando...</div>;
  if (articleQuery.error)
    return (
      <div className="text-center text-red-500 mt-10" role="alert">
        {(articleQuery.error as Error).message}
      </div>
    );
