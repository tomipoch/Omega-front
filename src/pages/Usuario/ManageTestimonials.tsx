...
import EditTestimonialModal from './ModalTestimonials';
...
const ManageTestimonials = () => {
  ...
  const fetchTestimonials = useCallback(
    async (stars: number | null, isOwn: boolean, page: number) => {
      setError('');
      setLoading(true);
      try {
        const params: TestimonioFilters = { limit: TESTIMONIALS_PER_PAGE, page };
        if (stars) params.stars = stars;
        if (isOwn && user?.usuario_id) params.usuario_id = user.usuario_id;

        const data = await obtenerTestimonios(params);
        const lista = extractList<Testimonio>(data);
        setTestimonials(lista);
        setTotalPages(lista.length < TESTIMONIALS_PER_PAGE ? page : page + 1);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (filterOwn && Number(user?.rol_id) === 2) {
      setError('Los administradores no tienen reseñas propias');
      setTestimonials([]);
      setLoading(false);
      return;
    }
    if (!filterOwn) setError('');
    fetchTestimonials(filterStars, filterOwn, currentPage);
  }, [filterStars, filterOwn, currentPage, token, user, authLoading, fetchTestimonials]);
  ...
  const handleSave = async (
    id: number,
    updated: { contenido: string; estrellas: number },
  ) => {
    try {
      const data = await actualizarTestimonio(id, updated);
      setTestimonials((prev) =>
        prev.map((t) =>
          t.testimonio_id === id ? { ...t, ...(data || updated) } : t,
        ),
      );
  ...
  const canSeeOwnReviews = Boolean(token && user && Number(user.rol_id) !== 2);
