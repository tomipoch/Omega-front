...
import { deleteAppointmentAdmin } from '../../services/appointmentsService';
...
const ManageDisponibilidad = () => {
  ...
  const availabilitiesQuery = useQuery<Disponibilidad[]>({
    queryKey: ['availabilities-admin', filtroEstado],
    queryFn: () => {
      const params: Record<string, string> = {};
      if (filtroEstado) params.estado = filtroEstado;
      return getAvailabilitiesAdmin(params);
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: DisponibilidadPayload) => createAvailability(data),
    onSuccess: () => {
      invalidate();
      resetForm();
    },
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: DisponibilidadPayload }) =>
  ...
  const deleteAppointmentMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (err: Error) => setError(`Error al eliminar cita: ${err.message}`),
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingDisponibilidad) {
      updateMutation.mutate({ id: editingDisponibilidad.disponibilidad_id, data: payload });
    } else {
  ...
  const handleDelete = async (id: number) => {
    const ok = await confirm({
      ...
      message: '¿Estás seguro de que quieres eliminar esta disponibilidad?',
  ...
  const handleEliminarCita = async (citaId: number) => {
    const ok = await confirm({
      title: 'Eliminar cita',
      message: '¿Estás seguro de que quieres eliminar esta cita? Esto liberará el horario.',
      confirmText: 'Eliminar',
      variant: 'danger',
    });
    if (!ok) return;
    deleteAppointmentMutation.mutate(citaId);
  };
  ...
  const loading = availabilitiesQuery.isLoading;

  const disponibilidadesAgrupadas = useMemo(
    () => groupByDate(disponibilidades),
    [disponibilidades],
  );
