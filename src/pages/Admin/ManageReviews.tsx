...
const AdminManageTestimonials = () => {
  ...
  const testimonialsQuery = useQuery<Testimonio[]>({
    ...
    queryFn: () => (activeTab === 'pendientes' ? getPendingReviews() : getAllReviews()),
  });

  const acceptMutation = useMutation({
    mutationFn: (id: number) => acceptReview(id),
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => deleteReviewAdmin(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['reviews'] }),
  });
  ...
  const handleDelete = async (id: number) => {
    const ok = await confirm({
      ...
      message: '¿Estás seguro de que quieres eliminar permanentemente esta reseña?',
  ...
  if (testimonialsQuery.isLoading) {
    return <p className="text-center text-gray-600">Cargando testimonios...</p>;
  }
  if (testimonialsQuery.error) {
    return (
      <p className="text-center text-red-500" role="alert">
        {(testimonialsQuery.error as Error).message}
      </p>
    );
  }
                  ...
                  <div className="flex justify-between items-center mt-4 space-x-2">
                    {activeTab === 'pendientes' ? (
                      <>
                        <button
                          type="button"
                          onClick={() => acceptMutation.mutate(testimonial.testimonio_id)}
                          disabled={acceptMutation.isPending}
                          className="bg-green-500 text-white py-1 px-3 text-sm rounded-lg shadow-md hover:bg-green-600 transition disabled:opacity-60"
                        >
                          Confirmar
                        </button>
                        <button
                          type="button"
                          onClick={() => rejectMutation.mutate(testimonial.testimonio_id)}
                          disabled={rejectMutation.isPending}
                          className="bg-red-500 text-white py-1 px-3 text-sm rounded-lg shadow-md hover:bg-red-600 transition disabled:opacity-60"
                        >
                          Rechazar
                        </button>
                      </>
                      ...
                      <button
                        ...
                        className="bg-red-600 text-white py-1 px-3 text-sm rounded-lg shadow-md hover:bg-red-700 transition w-full disabled:opacity-60"
                      >
